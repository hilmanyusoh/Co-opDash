...
def create_analysis_layout():
    ...
    return dbc.Container(
        children=[
            # Header
            html.Div([html.H1(" Dashboard วิเคราะห์ข้อมูลสมาชิก", 
            className="text-white text-center fw-bolder mb-0"), html.P("ภาพรวมและแนวโน้มที่สำคัญของข้อมูลสมาชิกทั้งหมด",
            ...
            # Charts Section
            dbc.Row([
                dbc.Col(dbc.Card([dbc.CardHeader([html.I(className="fas fa-chart-pie me-2 text-primary"), html.Span("1. ข้อมูลสัดส่วนจำนวนสมาชิกตามสาขา", 
                className="fw-bold fs-5"),], 
                className="bg-light py-3 border-bottom border-primary border-3"), 
                
                dbc.CardBody(dcc.Graph(figure=fig_branch, 
                                       style={'height': '450px'}))], 
                className="shadow-xl mb-4 h-100 rounded-4 overflow-hidden"), 
                        lg=6, md=12),
                
                dbc.Col(dbc.Card([dbc.CardHeader([html.I(className="fas fa-chart-bar me-2 text-info"), html.Span("2. ข้อมูลจำนวนสมาชิกตามช่วงอายุ", 
                className="fw-bold fs-5"),], 
                className="bg-light py-3 border-bottom border-info border-3"), 
            ...
            dbc.Row([
                dbc.Col(dbc.Card([dbc.CardHeader([html.I(
                className="fas fa-hand-holding-usd me-2 text-warning"), 
                                                  html.Span("3. ข้อมูลรายได้เฉลี่ยตามอาชีพ", 
                className="fw-bold fs-5"),], 
                className="bg-light py-3 border-bottom border-warning border-3"), 
                                  
                dbc.CardBody(dcc.Graph(figure=fig_income, style={'height': '450px'}))], 
                className="shadow-xl mb-4 h-100 rounded-4 overflow-hidden"), 
                        lg=6, md=12),
                
                dbc.Col(dbc.Card([dbc.CardHeader([html.I(
                className="fas fa-hourglass-half me-2 text-success"), html.Span("4. ข้อมูลระยะเวลาอนุมัติ", className="fw-bold fs-5"),], className="bg-light py-3 border-bottom border-success border-3"), dbc.CardBody(dcc.Graph(figure=fig_approval, style={'height': '450px'}))], 
                                 className="shadow-xl mb-4 h-100 rounded-4 overflow-hidden"), 
                        lg=6, md=12),
            ], className="g-4"),
            html.Br(), 
